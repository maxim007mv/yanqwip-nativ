...
import { FriendSuggestionsCard } from '@/components/FriendSuggestionsCard';
import { FriendProfileModal } from '@/components/FriendProfileModal';
import { FriendCategoriesModal } from '@/components/FriendCategoriesModal';
import { FriendRequestsList } from '@/components/FriendRequestsList';
import { FriendCategoriesList } from '@/components/FriendCategoriesList';
...
export default function ProfileScreen() {
  ...
  const [showFriendCategories, setShowFriendCategories] = useState(false);
  const [selectedFriendId, setSelectedFriendId] = useState<string | null>(null);
  const [friendsTab, setFriendsTab] = useState<'suggestions' | 'requests' | 'categories' | 'location'>('suggestions');
  ...
  const handleFriendCategoriesPress = () => {
    setShowFriendCategories(true);
  };

  const handleLocationSettingsPress = () => {
    setFriendsTab('location');
  };
  ...
  return (
          ...
          <View style={{ marginTop: Spacing.lg }}>
            ...
            <View style={styles.friendsTabs}>
              <TouchableOpacity
                style={[
                  styles.friendsTab,
                  friendsTab === 'suggestions' && styles.friendsTabActive,
                ]}
                onPress={() => setFriendsTab('suggestions')}
              >
                <Ionicons
                  name="people"
                  size={16}
                  color={friendsTab === 'suggestions' ? colors.accent : colors.text3}
                />
                <Text
                  style={[
                    styles.friendsTabText,
                    friendsTab === 'suggestions' && styles.friendsTabTextActive,
                    { color: friendsTab === 'suggestions' ? colors.accent : colors.text3 },
                  ]}
                  numberOfLines={1}
                >
              ...
              <TouchableOpacity
                style={[
                  styles.friendsTab,
                  friendsTab === 'requests' && styles.friendsTabActive,
                ]}
              ...
              >
                <Ionicons
                  name="mail"
                  size={16}
                  color={friendsTab === 'requests' ? colors.accent : colors.text3}
                />
                <Text
                  style={[
                    styles.friendsTabText,
                    friendsTab === 'requests' && styles.friendsTabTextActive,
                    { color: friendsTab === 'requests' ? colors.accent : colors.text3 },
                  ]}
                  numberOfLines={1}
                >
              ...
              <TouchableOpacity
                style={[
                  styles.friendsTab,
                  friendsTab === 'categories' && styles.friendsTabActive,
                ]}
                onPress={() => setFriendsTab('categories')}
              >
                <Ionicons
                  name="folder"
                  size={16}
                  color={friendsTab === 'categories' ? colors.accent : colors.text3}
                />
                <Text
                  style={[
                    styles.friendsTabText,
                    friendsTab === 'categories' && styles.friendsTabTextActive,
                    { color: friendsTab === 'categories' ? colors.accent : colors.text3 },
                  ]}
                  numberOfLines={1}
                >
              ...
              <TouchableOpacity
                style={[
                  styles.friendsTab,
                  friendsTab === 'location' && styles.friendsTabActive,
                ]}
                onPress={() => setFriendsTab('location')}
              >
                <Ionicons
                  name="location"
                  size={16}
                  color={friendsTab === 'location' ? colors.accent : colors.text3}
                />
                <Text
                  style={[
                    styles.friendsTabText,
                    friendsTab === 'location' && styles.friendsTabTextActive,
                    { color: friendsTab === 'location' ? colors.accent : colors.text3 },
                  ]}
                  numberOfLines={1}
                >
            ...
            {/* Контент таба */}
            {friendsTab === 'suggestions' && <FriendSuggestionsCard />}
            {friendsTab === 'requests' && (
              <FriendRequestsList onProfilePress={handleFriendPress} />
            )}
            {friendsTab === 'categories' && (
              <FriendCategoriesList onProfilePress={handleFriendPress} />
            )}
            {friendsTab === 'location' && (
              <GlassCard style={styles.locationCard} borderRadius="xxl">
                <Text style={[styles.locationTitle, { color: colors.text1 }]}>
                  Настройки локации
                </Text>
                <Text style={[styles.locationSubtitle, { color: colors.text2 }]}>
                  Управляйте тем, как друзья видят ваше местоположение
                </Text>
                <View style={styles.locationSettings}>
                  <View style={styles.locationSetting}>
                    <View style={styles.locationSettingInfo}>
                      <Ionicons name="location" size={20} color={colors.accent} />
                      <Text style={[styles.locationSettingTitle, { color: colors.text1 }]}>
                        Делиться локацией
                      </Text>
                    </View>
                    <Switch
                      ...
                      thumbColor={locationEnabled ? colors.text1 : colors.text3}
                    />
                  </View>
                  <Text style={[styles.locationSettingDescription, { color: colors.text3 }]}>
                    Позволяет друзьям видеть ваше местоположение в реальном времени
                  </Text>
                </View>
              </GlassCard>
            )}
    ...
    >
      <View style={styles.modalOverlay}>
        <View style={styles.friendSearchContainer}>
          <FriendSearchCard onClose={() => setShowFriendSearch(false)} />
        </View>
      </View>
    </Modal>
    ...
    {/* Модальное окно категорий друзей */}
    <FriendCategoriesModal
      visible={showFriendCategories}
      onClose={() => setShowFriendCategories(false)}
    />
    </>
  );
}
...
const styles = StyleSheet.create({
  ...
  friendsTab: {
    flex: 1,
    minWidth: 70, // Минимальная ширина для таба
    maxWidth: 120, // Максимальная ширина для предотвращения слишком широких табов
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
    paddingVertical: Spacing.sm,
  ...
  friendsTabText: {
    fontSize: 12, // Фиксированный меньший размер шрифта
    fontWeight: '500',
    flexShrink: 1,
    textAlign: 'center',
  },
